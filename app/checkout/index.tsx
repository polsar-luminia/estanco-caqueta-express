...
import { View, Text, ScrollView, TextInput, Pressable, KeyboardAvoidingView, Platform, AppState } from "react-native";
...
import { crearPedido, getDirecciones, crearDireccion, editarDireccion, validarCupon, getConfigApp, getEstadoTienda, ubicacionABody, validarCobertura, getFrioCarrito, getEtaActual, getMetodosPago, type DireccionGuardada, type CuponValidado, type UbicacionCapturada, type ApiError, type MetodoPago } from "../../src/lib/api";
...
import { ResumenPedidoFila } from "../../src/components/checkout/ResumenPedidoFila";
...
import {
  keyPedido,
  keyDireccion,
  direccionCreadaIdActual,
  marcarDireccionCreada,
  initiateCheckoutYaLogueado,
  marcarInitiateCheckoutLogueado,
  liberarIntento,
} from "../../src/lib/intentoPedido";
...
export default function CheckoutScreen() {
  ...
  // Precio/stock/cupo, ver useRefrescoCarrito: vive en las DOS pantallas, no
  // solo en la canasta (plan §Parte 2 · 4).
  useRefrescoCarrito();

  const [loading, setLoading] = useState(false);
  // Estado local, efímero: se pierde con gusto al desmontar el checkout
  // (hojas abiertas, borrador de la hoja de dirección, UI del cupón).
  ...
  // Borrador VISIBLE del checkout — vive en un store SIN persist (ver
  // src/stores/checkout.ts) para sobrevivir un ida-y-vuelta a la canasta:
  // sin esto, volver de "Editar" borraría el pin recién capturado en el
  // mapa, retiraría el cupón aplicado y haría reaparecer el modal de frío.
  // Nombrado IGUAL que el useState que reemplaza para no tocar el resto del
  // archivo (plan §Parte 2 · Paso 0).
  const mostrarNueva = useCheckoutStore((s) => s.mostrarNueva);
  const setMostrarNueva = useCheckoutStore((s) => s.setMostrarNueva);
  const nuevaDireccion = useCheckoutStore((s) => s.nuevaDireccion);
  const setNuevaDireccion = useCheckoutStore((s) => s.setNuevaDireccion);
  const nuevasNotas = useCheckoutStore((s) => s.nuevasNotas);
  const setNuevasNotas = useCheckoutStore((s) => s.setNuevasNotas);
  const nuevaUbicacion = useCheckoutStore((s) => s.nuevaUbicacion);
  const setNuevaUbicacion = useCheckoutStore((s) => s.setNuevaUbicacion);
  const permitirSinPin = useCheckoutStore((s) => s.permitirSinPin);
  const setPermitirSinPin = useCheckoutStore((s) => s.setPermitirSinPin);
  const notasOverride = useCheckoutStore((s) => s.notasOverride);
  const setNotasOverride = useCheckoutStore((s) => s.setNotasOverride);
  const quiereFrio = useCheckoutStore((s) => s.quiereFrio);
  const setQuiereFrio = useCheckoutStore((s) => s.setQuiereFrio);
  const medioPago = useCheckoutStore((s) => s.medioPago);
  const setMedioPago = useCheckoutStore((s) => s.setMedioPago);
  const codigoCupon = useCheckoutStore((s) => s.codigoCupon);
  const setCodigoCupon = useCheckoutStore((s) => s.setCodigoCupon);
  const cuponValidado = useCheckoutStore((s) => s.cuponValidado);
  const setCuponValidado = useCheckoutStore((s) => s.setCuponValidado);
  const usarPuntos = useCheckoutStore((s) => s.usarPuntos);
  const setUsarPuntos = useCheckoutStore((s) => s.setUsarPuntos);
  // Reemplazan cuponSubtotalRef / medioPagoInicializadoRef / recordatorioMostradoRef:
  // mismo motivo (sobrevivir el desmontaje), pero SÍ es estado de UI (afecta
  // qué se re-valida o se re-muestra), así que va en el store y no en
  // intentoPedido.ts.
  const cuponSubtotal = useCheckoutStore((s) => s.cuponSubtotal);
  const setCuponSubtotal = useCheckoutStore((s) => s.setCuponSubtotal);
  const medioPagoInicializado = useCheckoutStore((s) => s.medioPagoInicializado);
  const setMedioPagoInicializado = useCheckoutStore((s) => s.setMedioPagoInicializado);
  const recordatorioMostradoClave = useCheckoutStore((s) => s.recordatorioMostradoClave);
  const setRecordatorioMostradoClave = useCheckoutStore((s) => s.setRecordatorioMostradoClave);
  ...
  useEffect(() => {
    if (medioPagoInicializado || !medioPagoActivo || !cliente) return;
  ...
  }, [cliente, medioPagoActivo, pagoTarjetaActivo, cargandoMetodosPago, metodosPago, mediosPagoDisponibles, medioPagoInicializado, setMedioPago, setMedioPagoInicializado]);
  ...
  }, [medioPago, medioPagoEsTarjeta, tarjetaSeleccionadaId, medioPagoActivo, cargandoMetodosPago, metodosPago, mediosPagoDisponibles, setMedioPago]);
  ...
  // Espacio libre bajo el boton de confirmar. A diferencia de la canasta
  // (tab, con la barra flotante debajo), esta ruta vive FUERA de (tabs): no
  // hay tab bar que pagar, solo el inset real del dispositivo.
  const respiroBarra = insets.bottom + 12;
  ...
  }, [quiereFrio, hayElegibles, setQuiereFrio]);
  ...
  useEffect(() => {
    ...
    if (cuponSubtotal === subtotal) return;
    ...
    (async () => {
      try {
        ...
        if (cancelado) return;
        setCuponSubtotal(subtotalAtIntento);
        setCuponValidado(result);
      } catch {
        if (cancelado) return;
        setCuponSubtotal(null);
  ...
  }, [subtotal, cuponValidado, cuponSubtotal, setCodigoCupon, setCuponSubtotal, setCuponValidado]);

  const submitLockRef = useRef(false);
  // M-CART-15/M-CART-18/InitiateCheckout: keyPedido()/keyDireccion()/
  // direccionCreadaIdActual()/initiateCheckoutYaLogueado() en src/lib/intentoPedido.ts,
  // singleton de MÓDULO en vez de refs — sobrevive al desmontaje del checkout
  // (plan §Parte 2 · 2): un back a la canasta tras un timeout que SÍ creó el
  // pedido no debe generar una key nueva al reintentar.
  ...
  const handlePedir = async () => {
    ...
    const debeRecordar =
      ...
      hayElegibles &&
      !quiereFrio &&
      recordatorioMostradoClave !== claveCarrito;

    if (debeRecordar) {
      setRecordatorioMostradoClave(claveCarrito);
  ...
  const ejecutarPedido = async (datos: DatosPedido, conFrio: boolean) => {
    ...
    try {
      ...
      if (mostrarNueva && dirFinal && direccionCreadaIdActual() == null) {
        try {
          const nueva = await crearDireccion({ direccion: dirFinal, notas: notFinal || undefined, predeterminada: true, ...ubicacionABody(nuevaUbicacion) }, keyDireccion());
          marcarDireccionCreada(nueva.id);
      ...
      // Éxito: liberar el intento para que el próximo pedido empiece limpio
      liberarIntento();
      checkoutIniciadoRef.current = false;
      setRecordatorioMostradoClave(null);
      setMostrarRecordatorioFrio(false);
      setQuiereFrio(false);
  ...
  // El muro de registro real vive en la canasta (app/(tabs)/cart.tsx), que es
  // el único camino normal hacia aquí — ahí se mide registro_muro_mostrado.
  // Esto es solo el cinturón para un deep link directo a /checkout sin sesión:
  // no vuelve a emitir el evento, para no inflar el embudo con un caso que en
  ...
  // updateStocks puede vaciar el carrito ESTANDO en el checkout (un producto
  // que se agota se elimina, cart.ts) — el estado vacío no le pertenece a esta
  // pantalla; vuelve a la canasta, que sí sabe mostrarlo.
  if (items.length === 0) {
  ...
  return (
    ...
    <View className="flex-1" style={{ backgroundColor: colors.bg }}>
      {/* Header propio: esta ruta vive FUERA de (tabs) — no hay header nativo ni tab bar. */}
      <View style={{ flexDirection: "row", alignItems: "center", paddingTop: insets.top + 8, paddingHorizontal: 16, paddingBottom: 4 }}>
        <Pressable onPress={() => router.back()} accessibilityRole="button" accessibilityLabel="Volver a la canasta" hitSlop={10} style={{ padding: 4, marginRight: 8 }}>
          <Feather name="arrow-left" size={22} color={colors.ink} />
        </Pressable>
        <Text style={{ fontSize: 18, fontFamily: fuentes.titulo, color: colors.ink }}>Confirmar pedido</Text>
      </View>
      <ScrollView automaticallyAdjustKeyboardInsets keyboardDismissMode="interactive"
        contentContainerStyle={{ padding: 16, paddingTop: 8, paddingBottom: 24, gap: 12 }}
      >
        <ResumenPedidoFila nItems={items.length} subtotal={subtotal} onEditar={() => router.back()} />
